...
public class DownloadActivity extends DownloadBase {
    ...
    ServiceConnection mServiceConnectionSync = new ServiceConnection() {
            ...
            public void onServiceConnected(ComponentName name, IBinder service) {
                ...
                mDownloadCall = (DownloadCall)DownloadCall.Stub.asInterface(service);
            }
    ...
    public void runService(View view) {
        ...
        case R.id.bound_sync_button:
        	...
        	// In DownloadBase, strict mode is turned off and the code below works. 
        	// But if you re-enable it and try to run it again from the main UI thread, 
        	// you get a android.os.NetworkOnMainThreadException.
        	// The synchronous service call should be run in its own thread.
            ...
            if (mDownloadCall != null) {
            	// This is a synchronous call and we don't want to block 
            	// the UI thread. We use an AsyncTask to download the image 
            	// in a separate thread and then display it in the UI thread.
                new AsyncTask<Uri, Void, String>() {
                	...
                	// (and getting android.os.NetworkOnMainThreadException).
                	// When doInBackground finishes, it returns the image pathname
                	// which is passed to onPostExecute.
                    ...
                    // In an AsyncTask, the result of the computation run on a background thread 
                    // is published on the UI thread using onPostexecute
        ...
        case R.id.bound_async_button:
            ...
            if (mDownloadRequest != null) {
                try {
                    ...
                    // the client. The result is returned via the
                    // passed callback object (mDownloadCallback) sendPath() method 
                	// which is invoked by DownloadBoundServiceAsync to send back the
                    // pathname after having downloaded the image
