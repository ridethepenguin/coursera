...
public class LoginActivity extends StoryActivityBase{
	...
	public static File getLoginFile (Context context) {
		return StorageUtilities.getOutputMediaFile(context, 	// Line 48
				...
				/** The error stems from not using enum type in the security param. Best manner is using diferents 
				 * methods for security levels but in case of use only one function to all security levels it is
				 * better to use enum type instead of int type because it can ocurr this error: one developer set 
				 * two security levels (SECURITY_PRIVATE and SECURITY_PUBLIC) at StorageUtilities class but another
				 * developer use StorageUtilities.getOutputMediaFile but not those levels and that second developer
				 * create a new int value. The error is that in that function there is a if and MAX_SECURITY is 
				 * interpreted like public value.
				 * 
				 * To fix the error we only must to use the original security levels, that they behave correctly.
				 * In this case we want to write data securely so we must to use StorageUtilities.SECURITY_PRIVATE */
				/* MAX_SECURITY,*/ 
